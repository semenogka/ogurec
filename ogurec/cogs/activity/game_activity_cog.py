...
from ogurec.cogs.activity.game_activity_users import USERS_ID
...
class GameActivity(commands.Cog):
    def __init__(self, bot:OgurecBot, activity_storage: ActivityStorage):
        self.bot = bot
    ...
    async def on_presence_update(
        ...
        if old_game is not None and new_game is None: 
            ...
            if session:
                ...
                print(
                    f"{after} закончил играть в "
                    f"{session['game']} "
                    f"({duration})"
                )
                await self.activity_storage.end_game(
                    user_id=after.id,
                    ended_at=int(dt.now(TIME_ZONE).timestamp())
                )
        ...
        if new_game is not None:
            ...
            print(f"{after} начал играть в {new_game}")
